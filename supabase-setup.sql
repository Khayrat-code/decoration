...
-- ---- 4f. "complaints" + "contact" storage buckets (optional file attachments) --
-- Buckets can also be created via `npm run seed` (service-role key), but
-- creating them directly here means you don't need to run the script —
-- just paste this whole file in the SQL editor.
...
-- storage.objects RLS: anyone can upload/read an attachment, only the
